...
namespace McQLib.Recipes
{
    ...
    /// Discharge 레시피입니다.
    /// </summary>
    public sealed class Discharge : BaseConvertableRecipe
    {
        public override string GetManualString()
        {
            return "전지를 정전류(CC), 정전력(CP), 정저항(CR) 등의 방법으로 방전합니다.\r\n" +
                   "방전 중 시간, 전압, 전류, 온도 등을 모니터링하여 측정값을 이용해 방전 용량(Ah)이나 최대 전력 등의 파라미터 값들을 계산할 수 있습니다.";
        }
        ...
        public sealed override System.Drawing.Image Icon => Properties.Resources.Icon_Discharge;
        ...
        public override byte[] ToDataField( ushort stepNo, ushort endStepNo, ushort errorStepNo )
        {
            ...
            if ( !RecipeSetting.TransientResponse.Enabled ||     // TR 레시피가 비활성화된 상태인 경우 (지원하지 않는 장비)
                ...
                _sourcingType != SourcingType_Discharge.CC )    // TR Option이 ON이지만 SourcingType이 CC가 아닌 경우 (TR 옵션은 CC/CCCV 에서만 사용 가능함)
            {
        ...
        public override bool FromDataField( byte[] data )
        {
            ...
            // Mode2
            _sourcingType = ( SourcingType_Discharge )data[position++];
        ...
        [ID( "020000" )]
        public SourcingType_Discharge SourcingType
        {
            get
            {
        ...
        private SourcingType_Discharge _sourcingType = SourcingType_Discharge.CC;
