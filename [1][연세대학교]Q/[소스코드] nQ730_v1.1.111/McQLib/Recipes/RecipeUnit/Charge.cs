...
namespace McQLib.Recipes
{
    ...
    /// Charge 레시피입니다.
    /// </summary>
    public sealed class Charge : BaseConvertableRecipe
    {
        public override string GetManualString()
        {
            return "전지를 정전류(CC), 정전류/정전압(CCCV), 정전력(CP), 정저항(CR) 등의 방법으로 충전합니다.\r\n" +
        ...
        public sealed override System.Drawing.Image Icon => Properties.Resources.Icon_Charge;
        ...
        public override byte[] ToDataField(ushort stepNo, ushort endStepNo, ushort errorStepNo)
        {
            ...
            if (!RecipeSetting.TransientResponse.Enabled || // TR 레시피가 비활성화된 상태인 경우 (지원하지 않는 장비)
                ...
                (_sourcingType != SourcingType_Charge.CC && _sourcingType != SourcingType_Charge.CCCV))    // TR Option이 ON이지만 SourcingType이 CC/CCCV가 아닌 경우 (TR 옵션은 CC/CCCV 에서만 사용 가능함)
            {
        ...
        public override bool FromDataField(byte[] data)
        {
            ...
            // Mode2
            _sourcingType = (SourcingType_Charge)data[position++];
        ...
        private SourcingType_Charge _sourcingType = SourcingType_Charge.CC;
        ...
        //[Browsable( true )]
        //[ID( "0100010" )]
        //public SourcingType_CurrentUnit CurrentUnit { get; set; }
